...
sys.path.append("../")
model_name = "/data/huggingface/13B"
...
def get_upllama_output(prompt, args):
    return call_upllama(prompt, args.prompt_format, args.max_tokens, args.top_p, args.frequency_penalty, args.presence_penalty)

@lru_cache(maxsize=10000)
def call_upllama(prompt, prompt_format, max_tokens, top_p, frequency_penalty, presence_penalty):
    patience = 100
    while True:
        try:
            ...
            # print how many tokens were generated
            # print(f"tokens generated: {len(output[0])-len(prompt_tokens[0])}")
            output = tokenizer.decode(output[0], skip_special_tokens=True)
            prompt_end_idx = output.find(prompt) + len(prompt)
            output = output[prompt_end_idx:]
            if prompt_format == 'TQ-A':
                if "\n" in output:
                    output = output[:output.find("\n")]
            elif prompt_format == 'TQ-SA':
                if "The answer is " in output:
                    output = output[output.find("The answer is "):]
                if "\n" in output:
                    output = output[:output.find("\n")]

            break
        except Exception as e:
            logger.write(f"Exception: {e} in call_upllama")
            output = "nan"
...
def get_batch_reward_loss(scores, cand_pids, pid_batch, option_batch, unit_batch, label_batch, args):
    ...
    for i in range(len(scores)):
        ...
        # If the cnt of non-zero values are fewer than args.shot_number, then return nan for reward and loss. Stop training.
        if np.count_nonzero(cand_prob) < args.shot_number:
            print("!!! Fewer than args.shot_number non-zero values in cand_prob. Stop training.")
        ...
        output = get_upllama_output(prompt, args)
        ...
        # MIN_LOG_PROB = -1e10 # at least -1e5
        # log_prob = max(log_prob, MIN_LOG_PROB)
        
        print(f"log_prob:{log_prob}")
        print(f"prediction_norm.lower():{prediction_norm.lower()}")
        print(f"label_batch[i]:{label_batch[i].lower()}")
        print(prediction_norm.lower() == label_batch[i].lower())
...
def policy_gradient_train(policy_model, problems, train_pids, cand_pids, cand_examples, args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_i in range(num_batch):
            ...
            try : 
                if np.isnan(loss.item()) or np.isnan(reward):
                    STOP_FLAG = True
                    break
            except:
                if np.isnan(reward):
        ...
        # print cache info
        logger.write(call_upllama.cache_info())
        logger.write("============================================\n")
...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default='../dataset/tabmwp')
